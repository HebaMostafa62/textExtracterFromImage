...
# Random sample
np.random.seed(13)  # set seed for reproducibility #no
train_size = 1000
test_size = 102


rand_train_indices = np.random.choice(len(mnist.train.images), train_size, replace=False) #no
rand_test_indices = np.random.choice(len(mnist.test.images), test_size, replace=False)    #no
#dataset
